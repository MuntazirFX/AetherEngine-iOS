...
(synthetic X=0 split + marksurfaces; `vis_offset=-1` → all empty leaves visible), clipnode hulls for walk/slide/ground/step-up/crouch (distinct crouch Z, jump ceiling clamp, low alcove duck), and red monster debug boxes from the entity/monster registries.
...
- `AetherCollision` — clipnode hull point-in-solid + move-and-slide + step-up + crouch/stand hull heights (synthetic room, 16u ledge, low alcove)
